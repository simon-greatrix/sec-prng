...
/**
 * An implementation of the NIST Hash-based Deterministic Random Number Generator as defined in SP800-90A.
 *
...
public class NistHashRandom extends BaseRandom {
  ...
  public static class RandomSHA1 extends NistHashRandom {
    ...
    public RandomSHA1(SecureRandomParameters parameters) {
      super(null, HashSpec.SPEC_SHA1, 0, null, null, BaseRandom.getPersonalization(parameters));
      BaseRandom.verifyStrength(parameters, 128);
    }
  ...
  public static class RandomSHA256 extends NistHashRandom {
    ...
    public RandomSHA256(SecureRandomParameters parameters) {
      super(null, HashSpec.SPEC_SHA256, 0, null, null, BaseRandom.getPersonalization(parameters));
      BaseRandom.verifyStrength(parameters, 256);
    }
  ...
  public static class RandomSHA512 extends NistHashRandom {
    ...
    public RandomSHA512(SecureRandomParameters parameters) {
      super(null, HashSpec.SPEC_SHA512, 0, null, null, BaseRandom.getPersonalization(parameters));
      // SHA-512 is still only 256 bit strong according to NIST
      BaseRandom.verifyStrength(parameters, 256);
    }
   ...
   */
  public NistHashRandom(
      SeedSource source, HashSpec spec, int resistance,
      byte[] entropy, byte[] nonce, byte[] personalization
  ) {
    super(source, new InitialMaterial(source, entropy, nonce, personalization, spec.seedLength, spec.seedLength), resistance, spec.seedLength,
        spec.outputLength
    );
    this.spec = spec;
  ...
  /**
   * The NIST hashDF function which is used to update this PRNGs state with external data.
   *
   * @param zeroPrefix if true, a zero is prefixed to the material
   * @param material   the hash material
   *
   ...
   *
   * @param output array to place output into
   * @param off    where to start output
   * @param len    amount of bytes to output
   */
